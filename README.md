...
# --inner join (rows match in all tables)
...
# --left_join(rows from the customers table even if they don't match with the ones in the other tables ) 
...
# --Right_join( rows from product even if no orders were made) 
...
For example here we have 3 classes where each has two rows.*/
  ...
  select product_name,prod_price ,NTILE(3) OVER (ORDER BY prod_price DESC) from products;
